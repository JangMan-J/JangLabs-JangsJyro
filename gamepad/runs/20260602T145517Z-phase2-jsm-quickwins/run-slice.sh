#!/usr/bin/env bash
# run-slice.sh <slice> — one JSM-lane tracer slice: synthetic trace -> JSM -> evdev.
# Reads configs/<slice>.cfg + traces/<slice>.trace; writes <slice>.{capture.txt,
# capture.jsonl,injector.log,jsm.stdout.log}. Serial run-and-observe (one live system).
set -u
SLICE="$1"
ROOT=/home/jangmanj/JangLabs/jangsjyro
RUN="$ROOT/gamepad/runs/20260602T145517Z-phase2-jsm-quickwins"
...
sleep 1.0
echo "[$SLICE] grab-capture JSM output (16s)"
python3 "$TOOLS/evdev_capture.py" capture --name JoyShockMapper --grab-name JoyShockMapper \
    --types key,rel,syn --seconds 16 --jsonl "$RUN/$SLICE.capture.jsonl" \
